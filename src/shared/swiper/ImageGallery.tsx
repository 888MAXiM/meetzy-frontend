    ...
    <div className={`image-modal-overlay ${className}`} onClick={onClose}>
      <div className="gallery-container relative" onClick={(e) => e.stopPropagation()}>
        ...
        {media.length > 0 ? (
          ...
          >
            {media.map((item) => (
              <SwiperSlide key={item.messageId || item.src}>
                {item.type === 'video' ? (
                  <video controls className="img-fluid" onError={() => console.warn('Video failed to load:', item.src)}>
          ...
          <video controls className="img-fluid" onError={() => console.warn('Video failed to load:', media[0]?.src)}>
